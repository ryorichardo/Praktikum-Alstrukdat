...
// Kelompok baca tulis tidak dibuat tcnya
...
// Output: M3 = M1 + M2
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      ck_assert_int_eq(Elmt(M3, i, j), Elmt(M1, i, j) + Elmt(M2, i, j));
    }
  }
} END_TEST
...
// Output: M3 = M1 * M2
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      el = 0;
      for(k = KolMin; k < BrsMin+3; k++){
        el += Elmt(M1, i, k) * Elmt(M2, k , j);
      }
      ck_assert_int_eq(Elmt(M3, i, j), el);
    }
  }
} END_TEST
...
// Output: M2 = M1 * p
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      ck_assert_int_eq(Elmt(M2, i, j), Elmt(M1, i, j) * 3);
    }
  }
} END_TEST
...
// Output: M1 = M1 * p
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      // Ubah (i*3+j) menjadi nilai awal dari matriks
      ck_assert_int_eq(Elmt(M1, i, j), (i*3+j) * 3);
    }
  }
} END_TEST
...
// Output: M2 = M1 * -1
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      ck_assert_int_eq(Elmt(M2, i, j), Elmt(M1, i, j) * -1);
    }
  }
} END_TEST
...
// Output: M = M * -1
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      ck_assert_int_eq(Elmt(M, i, j), (i*3+j) * -1);
    }
  }
} END_TEST
...
// Output: M2 = Transpose(M1)
  ...
  for(i = BrsMin; i < BrsMin+3; i++){
    for(j = KolMin; j < KolMin+3; j++){
      ck_assert_int_eq(Elmt(M1, i, j), Elmt(M2, j, i));
    }
  }
} END_TEST
